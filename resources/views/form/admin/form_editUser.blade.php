            ...
            <div class="conten_box">
                ...
                <div class="body_box home1">
                    <div class="conten_body">
                        ...
                        <form action="{{route('updatePersonnel',$dataUser->personnel->id)}}" method="post" class="add_personnel" enctype="multipart/form-data">
                            @csrf
                            <div class="form">
                                <div class="mb_3">
                                ...
                                <input type="year" name="age" class="input_user" value="{{$dataUser->personnel->age}}">
                                @error('age')
                                    <span class="error lg">{{$message}}</span>
                                @enderror
                                <label for="" class="name_input">image:</label>
                                <img src="/upload/{{$dataUser->personnel->image}}" alt="" width="150" height="150">
                                <input type="file" name="image" class="input_user" value="{{$dataUser->personnel->image}}">
                                @error('image')
                                    <span class="error lg">{{$message}}</span>
                                @enderror
                                <label for="" class="name_input">number phone:</label>
                                <input type="text" class="input_user" name="number_phone" value="{{$dataUser->personnel->number_phone}}">
                                @error('number_phone')
                                    <span class="error lg">{{$message}}</span>
                                @enderror
                                <label for="" class="name_input">address:</label>
                                <input type="text" name="address" value="{{$dataUser->personnel->address}}" class="input_user">
                                @error('number_phone')
                                    <span class="error lg">{{$message}}</span>
                                @enderror
                            ...
                            <div class="footer_login_auth">
                                <button type="submit" class="btn_input">edit user</button>
                            </div>
                        </form>
                        @else
